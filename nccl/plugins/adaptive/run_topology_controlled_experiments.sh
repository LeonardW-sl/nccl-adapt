...
RUN_LABEL="${RUN_LABEL:-$(date +%F)-topology-controlled-matrix}"
RESULT_ROOT="${RESULT_ROOT:-${SCRIPT_DIR}/experiments/topology-controlled/${RUN_LABEL}}"
CONTAINER_RESULT_ROOT="${CONTAINER_PLUGIN_DIR}/experiments/topology-controlled/${RUN_LABEL}"

REPLICATES="${REPLICATES:-3}"
MODE_ORDER_STRATEGY="${MODE_ORDER_STRATEGY:-rotate}"
MODE_ORDER_SEED="${MODE_ORDER_SEED:-20260511}"
MODE_MESSAGE_MB="${MODE_MESSAGE_MB:-8}"
MODE_WARMUP_ITERS="${MODE_WARMUP_ITERS:-4}"
MODE_MEASURE_ITERS="${MODE_MEASURE_ITERS:-72}"
SIZE_SWEEP_MBS="${SIZE_SWEEP_MBS:-5,6,7}"
MODE_RECHECK_AFTER="${MODE_RECHECK_AFTER:-64}"
SIZE_SWEEP_RECHECK_AFTER="${SIZE_SWEEP_RECHECK_AFTER:-8}"
RECHECK_SHIFT_GROUP="${RECHECK_SHIFT_GROUP:-cross-8gpu}"
RECHECK_SHIFT_VALUES="${RECHECK_SHIFT_VALUES:-32}"
...
write_matrix_manifest() {
  ...
  SIZE_SWEEP_RECHECK_AFTER="${SIZE_SWEEP_RECHECK_AFTER}" \
  RECHECK_SHIFT_GROUP="${RECHECK_SHIFT_GROUP}" \
  RECHECK_SHIFT_VALUES="${RECHECK_SHIFT_VALUES}" \
...
recheck_shift_values = [
    int(item) for item in os.environ["RECHECK_SHIFT_VALUES"].split(",") if item.strip()
]
...
payload = {
    ...
    "parameters": {
        ...
        "size_sweep_mbs": os.environ["SIZE_SWEEP_MBS"],
        "size_sweep_recheck_after": int(os.environ["SIZE_SWEEP_RECHECK_AFTER"]),
    },
    "groups": {
        "numa0-4gpu": {
            ...
            "numa_nodes": "0",
            "cross_socket": False,
            "smoke_dir": "smoke-baseline/numa0-4gpu",
            "mode_comparison_dir": "mode-comparison/numa0-4gpu",
            "size_sweep_dir": "state-granularity/numa0-4gpu",
        },
        "numa1-4gpu": {
            ...
            "numa_nodes": "1",
            "cross_socket": False,
            "smoke_dir": "smoke-baseline/numa1-4gpu",
            "mode_comparison_dir": "mode-comparison/numa1-4gpu",
            "size_sweep_dir": "state-granularity/numa1-4gpu",
        },
        "cross-8gpu": {
            ...
            "numa_nodes": "0,1",
            "cross_socket": True,
            "smoke_dir": "smoke-baseline/cross-8gpu",
            "mode_comparison_dir": "mode-comparison/cross-8gpu",
            "size_sweep_dir": "state-granularity/cross-8gpu",
        },
    },
    "recheck_shift": {
        "group": os.environ["RECHECK_SHIFT_GROUP"],
        "variants": [
            {
                "label": f"recheck-after-{os.environ['MODE_RECHECK_AFTER']}",
                "recheck_after": int(os.environ["MODE_RECHECK_AFTER"]),
                "result_dir": f"recheck-shift/{os.environ['RECHECK_SHIFT_GROUP']}/recheck-after-{os.environ['MODE_RECHECK_AFTER']}",
            },
            *[
                {
                    "label": f"recheck-after-{value}",
                    "recheck_after": value,
                    "result_dir": f"recheck-shift/{os.environ['RECHECK_SHIFT_GROUP']}/recheck-after-{value}",
                }
                for value in recheck_shift_values
            ],
        ],
    },
}

with open(os.path.join(result_root, "matrix-manifest.json"), "w", encoding="utf-8") as handle:
...
main() {
  ...
  local groups=(numa0-4gpu numa1-4gpu cross-8gpu)
  local group
  for group in "${groups[@]}"; do
    run_batch \
      ...
      "${group}" \
      mode-comparison \
      "MODE_SEQUENCE=baseline" \
      "NCCL_ADAPTIVE_RECHECK_AFTER=${MODE_RECHECK_AFTER}"
  done
  ...
  for group in "${groups[@]}"; do
    run_batch \
      ...
      "NCCL_ADAPTIVE_RECHECK_AFTER=${SIZE_SWEEP_RECHECK_AFTER}" \
      "NCCL_ADAPTIVE_LOG_COORDINATOR=1"
  done

  run_batch \
    "recheck-shift/${RECHECK_SHIFT_GROUP}/recheck-after-${MODE_RECHECK_AFTER}" \
    "${RECHECK_SHIFT_GROUP}" \
    mode-comparison \
    "MODE_SEQUENCE=weak-online" \
    "NCCL_ADAPTIVE_RECHECK_AFTER=${MODE_RECHECK_AFTER}"

  local recheck_value
  IFS=',' read -r -a RECHECK_VARIANTS <<<"${RECHECK_SHIFT_VALUES}"
  for recheck_value in "${RECHECK_VARIANTS[@]}"; do
    [[ -z "${recheck_value}" ]] && continue
    run_batch \
      "recheck-shift/${RECHECK_SHIFT_GROUP}/recheck-after-${recheck_value}" \
      "${RECHECK_SHIFT_GROUP}" \
      mode-comparison \
      "MODE_SEQUENCE=weak-online" \
      "NCCL_ADAPTIVE_RECHECK_AFTER=${recheck_value}"
  done

  python3 "${ANALYZER}" aggregate-topology-matrix \
    --manifest "${RESULT_ROOT}/matrix-manifest.json" \
    --output "${RESULT_ROOT}/topology-summary.json"
}
