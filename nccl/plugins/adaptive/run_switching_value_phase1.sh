...
RUN_LABEL="${RUN_LABEL:-$(date +%F)-switching-value-phase1}"
RESULT_ROOT="${RESULT_ROOT:-${SCRIPT_DIR}/experiments/switching-value/phase1/${RUN_LABEL}}"
CONTAINER_RESULT_ROOT="${CONTAINER_PLUGIN_DIR}/experiments/switching-value/phase1/${RUN_LABEL}"

REPLICATES="${REPLICATES:-3}"
MODE_ORDER_STRATEGY="${MODE_ORDER_STRATEGY:-rotate}"
MODE_ORDER_SEED="${MODE_ORDER_SEED:-20260513}"
MODE_SEQUENCE="${MODE_SEQUENCE:-baseline,static-replay,final-steady,weak-online}"
PRIMARY_STATIC_REPLAY_CANDIDATE="${PRIMARY_STATIC_REPLAY_CANDIDATE:-ring/simple}"
SIZE_SET_MBS="${SIZE_SET_MBS:-4,8,16,32,64,128,256}"
MODE_WARMUP_ITERS="${MODE_WARMUP_ITERS:-4}"
MODE_MEASURE_ITERS="${MODE_MEASURE_ITERS:-192}"
MODE_RECHECK_AFTER="${MODE_RECHECK_AFTER:-64}"
BATCH_A_GROUP="${BATCH_A_GROUP:-numa1-4gpu}"
BATCH_B_PRIMARY_GROUP="${BATCH_B_PRIMARY_GROUP:-cross-8gpu}"
BATCH_B_BOUNDARY_GROUP="${BATCH_B_BOUNDARY_GROUP:-numa0-4gpu}"
RUN_BOUNDARY_GROUP="${RUN_BOUNDARY_GROUP:-0}"
POSITIVE_EVIDENCE_THRESHOLD_PCT="${POSITIVE_EVIDENCE_THRESHOLD_PCT:-2.0}"
...
group_cuda_visible_devices() {
  case "$1" in
    ...
    cross-8gpu) printf '0,1,2,3,4,5,6,7' ;;
    *)
...
group_cross_socket() {
  case "$1" in
    numa0-4gpu|numa1-4gpu) printf 'false' ;;
    cross-8gpu) printf 'true' ;;
    *)
      echo "unknown topology group: $1" >&2
      exit 1
      ;;
...
ldd --version > "${CONTAINER_RESULT_ROOT}/container/ldd-version.txt"
ldd "${EXPECTED_PLUGIN_PATH}" > "${CONTAINER_RESULT_ROOT}/container/ldd-libnccl-adaptive.txt"
readelf -d "${EXPECTED_PLUGIN_PATH}" > "${CONTAINER_RESULT_ROOT}/container/readelf-libnccl-adaptive.txt"
nm -D --defined-only "${EXPECTED_PLUGIN_PATH}" | grep -E 'ncclProfiler_v5|ncclTunerPlugin_v5' \
  > "${CONTAINER_RESULT_ROOT}/container/nm-libnccl-adaptive.txt"
EOF
)")
  "${docker_cmd[@]}"
}

run_mode_comparison_batch() {
  local relative_result_dir=$1
  local topology_group=$2
  local message_mb=$3
  local mode_sequence=$4
  local static_replay_candidate=$5

  local cuda_visible_devices nproc_per_node cpu_affinity numa_nodes cross_socket batch_label
  ...
  cross_socket=$(group_cross_socket "${topology_group}")
  ...
  docker_cmd+=(
    ...
    -e "ADAPTIVE_MESSAGE_MB=${message_mb}"
    ...
    -e "NCCL_ADAPTIVE_RECHECK_AFTER=${MODE_RECHECK_AFTER}"
    ...
    -e "TOPOLOGY_CROSS_SOCKET=${cross_socket}"
...
maybe_run_candidate_aligned_replay() {
  local primary_dir=$1
  local topology_group=$2
  local message_mb=$3

  local comparison_summary="${RESULT_ROOT}/${primary_dir}/comparison-summary.json"
  mapfile -t alignment_info < <(read_alignment_state "${comparison_summary}" "${PRIMARY_STATIC_REPLAY_CANDIDATE}")
  local alignment_status="${alignment_info[0]:-unknown}"
  local learned_candidate="${alignment_info[1]:-}"

  if [[ "${alignment_status}" == "mismatch" && -n "${learned_candidate}" && "${learned_candidate}" != *","* ]]; then
    local aligned_root
    aligned_root="$(dirname "${primary_dir}")/candidate-aligned-replay/$(sanitize_candidate_name "${learned_candidate}")"
    run_mode_comparison_batch \
      "${aligned_root}" \
      "${topology_group}" \
      "${message_mb}" \
...
write_phase1_manifest() {
  ...
  BATCH_A_GROUP="${BATCH_A_GROUP}" \
  BATCH_B_PRIMARY_GROUP="${BATCH_B_PRIMARY_GROUP}" \
  BATCH_B_BOUNDARY_GROUP="${BATCH_B_BOUNDARY_GROUP}" \
  RUN_BOUNDARY_GROUP="${RUN_BOUNDARY_GROUP}" \
  POSITIVE_EVIDENCE_THRESHOLD_PCT="${POSITIVE_EVIDENCE_THRESHOLD_PCT}" \
  python3 - <<'PY'
import json
import os

sizes = [int(item) for item in os.environ["SIZE_SET_MBS"].split(",") if item.strip()]

def size_entry(batch: str, topology: str, size_mb: int) -> dict[str, object]:
    size_label = f"{size_mb}m"
    base = f"phase1/{batch}/{topology}/{size_label}"
    return {
        "size_mb": size_mb,
        "size_label": size_label,
        "primary_dir": f"{base}/primary",
        "candidate_aligned_root": f"{base}/candidate-aligned-replay",
    }
...
payload = {
    ...
    "container_metadata": {
        ...
        "ldd_version": "container/ldd-version.txt",
        "ldd_plugin": "container/ldd-libnccl-adaptive.txt",
        "readelf_plugin": "container/readelf-libnccl-adaptive.txt",
        "nm_plugin": "container/nm-libnccl-adaptive.txt",
    },
    "parameters": {
        "replicates": int(os.environ["REPLICATES"]),
        "mode_sequence": os.environ["MODE_SEQUENCE"],
        "primary_static_replay_candidate": os.environ["PRIMARY_STATIC_REPLAY_CANDIDATE"],
        "size_set_mbs": sizes,
        "mode_warmup_iters": int(os.environ["MODE_WARMUP_ITERS"]),
        "mode_measure_iters": int(os.environ["MODE_MEASURE_ITERS"]),
        "mode_recheck_after": int(os.environ["MODE_RECHECK_AFTER"]),
        "mode_order_strategy": os.environ["MODE_ORDER_STRATEGY"],
        "mode_order_seed": int(os.environ["MODE_ORDER_SEED"]),
        "positive_evidence_threshold_pct": float(os.environ["POSITIVE_EVIDENCE_THRESHOLD_PCT"]),
        ...
        "candidate_alignment_policy": "supplement-static-replay-when-final-steady-learns-a-different-unique-candidate",
    },
    "phase1": {
        "batch_a": {
            "topology_group": os.environ["BATCH_A_GROUP"],
            "entries": [size_entry("batch-a", os.environ["BATCH_A_GROUP"], size_mb) for size_mb in sizes],
        },
        "batch_b": {
            "primary_topology_group": os.environ["BATCH_B_PRIMARY_GROUP"],
            "boundary_topology_group": os.environ["BATCH_B_BOUNDARY_GROUP"],
            "run_boundary_group": os.environ["RUN_BOUNDARY_GROUP"] == "1",
            "primary_entries": [
                size_entry("batch-b", os.environ["BATCH_B_PRIMARY_GROUP"], size_mb) for size_mb in sizes
            ],
            "boundary_entries": [
                size_entry("batch-b", os.environ["BATCH_B_BOUNDARY_GROUP"], size_mb) for size_mb in sizes
            ],
        },
    },
}

with open(os.path.join(os.environ["RESULT_ROOT"], "phase1-manifest.json"), "w", encoding="utf-8") as handle:
...
with open(sys.argv[1], "r", encoding="utf-8") as handle:
    payload = json.load(handle)
for item in payload.get("batch_a", {}).get("promoted_sizes", []):
    print(str(item))
PY
}

run_batch_a() {
  split_csv "${SIZE_SET_MBS}"
  local size_mb size_dir
  for size_mb in "${SPLIT_RESULT[@]}"; do
    [[ -z "${size_mb}" ]] && continue
    size_dir="phase1/batch-a/${BATCH_A_GROUP}/$(size_label "${size_mb}")/primary"
    run_mode_comparison_batch "${size_dir}" "${BATCH_A_GROUP}" "${size_mb}" "${MODE_SEQUENCE}" "${PRIMARY_STATIC_REPLAY_CANDIDATE}"
    maybe_run_candidate_aligned_replay "${size_dir}" "${BATCH_A_GROUP}" "${size_mb}"
  done
}

run_batch_b() {
  local summary_path=$1
  local size_mb primary_dir
  while IFS= read -r size_mb; do
    [[ -z "${size_mb}" ]] && continue
    primary_dir="phase1/batch-b/${BATCH_B_PRIMARY_GROUP}/$(size_label "${size_mb}")/primary"
    run_mode_comparison_batch "${primary_dir}" "${BATCH_B_PRIMARY_GROUP}" "${size_mb}" "${MODE_SEQUENCE}" "${PRIMARY_STATIC_REPLAY_CANDIDATE}"
    maybe_run_candidate_aligned_replay "${primary_dir}" "${BATCH_B_PRIMARY_GROUP}" "${size_mb}"

    if [[ "${RUN_BOUNDARY_GROUP}" == "1" ]]; then
      primary_dir="phase1/batch-b/${BATCH_B_BOUNDARY_GROUP}/$(size_label "${size_mb}")/primary"
      run_mode_comparison_batch "${primary_dir}" "${BATCH_B_BOUNDARY_GROUP}" "${size_mb}" "${MODE_SEQUENCE}" "${PRIMARY_STATIC_REPLAY_CANDIDATE}"
      maybe_run_candidate_aligned_replay "${primary_dir}" "${BATCH_B_BOUNDARY_GROUP}" "${size_mb}"
    fi
  done < <(read_promoted_sizes "${summary_path}")
}
...
main() {
  ...
  python3 "${ANALYZER}" aggregate-switching-value-phase1 \
    --manifest "${RESULT_ROOT}/phase1-manifest.json" \
    --output "${RESULT_ROOT}/phase1-summary.json"

  run_batch_b "${RESULT_ROOT}/phase1-summary.json"
